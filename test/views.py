...
from .models import test,likes,mass_post
import base64
...
def test(request):
    if request.method == 'POST':
        ...
        email_valid = test.objects.filter(email_id__icontains=email_id)
...
def test(request):
    if request.method == 'POST':
        ...
        tst = test.objects.filter(email_id__icontains=email_id,
                                                               password__icontains=password)
        if tst.exists():
            login_data = test.objects.filter(email_id__icontains=email_id,
                                                               password__icontains=password)
